...
update_aitoolkit_model_links() {
    ...
    for model in "${models[@]}"; do
        ...
        local target_path="/root/autodl-tmp/${model}"
...
update_flux2_klein_vae_links() {
    ...
    for model in "${flux2_klein_models[@]}"; do
        # 目标路径和源路径
        local target_model_path="/root/autodl-tmp/${model}"
...
update_accuracy_recovery_adapters() {
    ...
    # 精度恢复适配器列表（源路径 -> 目标路径）
    # 格式：源路径在 /.autodl/ 下（使用哈希路径），目标路径在 /root/autodl-tmp/ostris/accuracy_recovery_adapters/ 下
    local adapters=(
    ...
    # 创建目标目录
    local target_dir="/root/autodl-tmp/ostris/accuracy_recovery_adapters"
    mkdir -p "$target_dir"
...
update_training_adapters() {
    ...
    # 创建目标目录
    local target_dir="/root/autodl-tmp/ostris/zimage_turbo_training_adapter"
    mkdir -p "$target_dir"
